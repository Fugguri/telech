...
class Database:
    ...
    def create_user(self, telegram_id, full_name: str, username: str,):
        ...
        with self.connection.cursor() as cursor:
            sql = 'INSERT IGNORE INTO users (telegram_id, full_name, username) VALUES(%s, %s, %s)'
            cursor.execute(sql, (telegram_id, full_name, username))
            self.connection.commit()
    ...
    def create_chat_or_channel(self, chat_link):
        self.connection.ping()
        with self.connection.cursor() as cursor:
            sql = 'INSERT IGNORE INTO chats_or_channels (chat_link) VALUES(%s)'
            cursor.execute(sql, (chat_link,))
            self.connection.commit()
            self.connection.close()
    ...
    def all_chats(self, telegram_id) -> tuple:
        self.connection.ping()
        with self.connection.cursor() as cursor:
    ...
    def add_event(self, chat_id, user_id, username, full_name, status, date, subs_amount):
        ...
        with self.connection.cursor() as cursor:
            ...
            sql = """INSERT IGNORE INTO """+table_name + \
                """ (user_id, username, full_name, status, date, subs_amount) VALUES (%s,%s,%s,%s,%s,%s)"""
            cursor.execute(sql, (
                user_id,
                username,
                full_name,
    ...
    def user_events(self, user_id: int) -> dict:
        result = dict()
        self.connection.ping()
        with self.connection.cursor() as cursor:
            sql = """SELECT chat_id,chat_name FROM chats_or_channels WHERE id in 
            (SELECT chat_id FROM users_chats WHERE user_id = (SELECT user_id FROM users WHERE telegram_id=(%s)))"""
